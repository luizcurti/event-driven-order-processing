...
const EMF_NAMESPACE = 'OrderProcessing';

/**
 * Emits invocation count/duration as a CloudWatch Embedded Metric Format
 * (EMF) log line. CloudWatch Logs parses this into custom metrics on its
 * own, so this needs no PutMetricData call and no IAM permission beyond the
 * logs:PutLogEvents every Lambda execution role already has.
 */
export const emitEmfMetric = (
  serviceName: string,
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      Outcome: outcome,
      InvocationCount: 1,
      InvocationDuration: durationSeconds * 1000
    })}\n`
  );
};

/**
 * Reports per-invocation metrics for ephemeral Lambda executions, which
 * can't be scraped (pull-based) since the process exits right after the
 * invocation. When PUSHGATEWAY_URL is set (the LocalStack real-Lambda dev
 * path, see the README's local observability section) it pushes to
 * Pushgateway/Prometheus, since that's what the local Grafana stack
 * scrapes. Otherwise it reports via emitEmfMetric, but only when actually
 * running inside a Lambda runtime (AWS_LAMBDA_FUNCTION_NAME, set by AWS
 * itself) -- this keeps it a no-op for unit tests and the direct-call shim,
 * same as before this function grew a non-Pushgateway path.
 */
export const pushInvocationMetrics = async (
  ...
  if (!pushgatewayUrl) {
    if (process.env.AWS_LAMBDA_FUNCTION_NAME) {
      emitEmfMetric(serviceName, durationSeconds, outcome);
    }
    return;
  }
